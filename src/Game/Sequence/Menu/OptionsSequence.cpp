...
namespace
{
    bool IsMSAAModEnabled(void)
    {
#if defined(TARGET_PC)
        return CPCModFeatures::IsMSAAEnabled();
    ...
    bool IsDifficultyModEnabled(void)
    {
#if defined(TARGET_PC)
...
static const FontData_t s_aDisplayFontMSAA[] =
{
    { true, GAMETEXT_OP_DISP_HITFX, { FONT_DISPLAY_X_POS(0), FONT_DISPLAY_Y_POS(0) }, FONT_HEIGHT_SCALE, s_ColorOrange, nullptr },
    { true, GAMETEXT_OP_DISP_MARK,  { FONT_DISPLAY_X_POS(1), FONT_DISPLAY_Y_POS(1) }, FONT_HEIGHT_SCALE, s_ColorOrange, nullptr },
    { true, GAMETEXT_OP_DISP_HELP,  { FONT_DISPLAY_X_POS(2), FONT_DISPLAY_Y_POS(2) }, FONT_HEIGHT_SCALE, s_ColorOrange, nullptr },
...
static OPTIONMODE s_aNextModeDisplayMSAA[] =
{
    OPTIONMODE_EFFECT,
    OPTIONMODE_MARKER,
    OPTIONMODE_WINDOW,
...
void COptions::SettingInit(bool bDisplayChanged)
{
    ...
    m_MultiSamplingSamplesCur =
        (IsMSAAModEnabled() ?
         CGameData::Option().Display().GetMultiSamplingSamples() : 0);
    ...
    case OPTIONMODE_OPTIONS:
        {
            ...
            if ((m_eOptionModeNext == OPTIONMODE_DISPLAY) && IsMSAAModEnabled())
            {
                m_pFontDataNext = s_aDisplayFontMSAA;
                m_IndexMaxNext = COUNT_OF(s_aDisplayFontMSAA) - 2;
            };
    ...
    case OPTIONMODE_DISPLAY:
        {
            ...
            const FontData_t* pDisplayFont =
                (IsMSAAModEnabled() ? s_aDisplayFontMSAA : s_aDisplayFont);
            const OPTIONMODE* pNextModeDisplay =
                (IsMSAAModEnabled() ? s_aNextModeDisplayMSAA : s_aNextModeDisplay);
            const int32 displayIndexMax =
                (IsMSAAModEnabled() ?
                 COUNT_OF(s_aDisplayFontMSAA) - 2 :
                 COUNT_OF(s_aDisplayFont) - 2);

            FontDataClear();
            FontDataSet(pDisplayFont);
            
            m_IndexMax          = displayIndexMax;
            m_eOptionModeNext   = pNextModeDisplay[m_IndexLine];
            m_IndexLineNext     = m_IndexLine;
            m_pFontDataNext     = s_aOptionFont;
            m_EffectTime        = (m_IndexLine != m_IndexMax ? 0 : SCROLL_TIME);
            m_IndexMaxNext      = COUNT_OF(s_aOptionFont) - 2;
    ...
    case OPTIONMODE_MSAA:
        {
            if (!IsMSAAModEnabled())
            {
                m_eOptionMode = OPTIONMODE_DISPLAY;
                m_IndexLine = 0;
                break;
            };
    ...
    case OPTIONMODE_DISPLAY_OK:
        {
            ...
            bool bMultiSamplingChanged = IsMSAAModEnabled() &&
...
void COptions::SwitchDifficuly(int32 Line)
{
    ...
    if (!IsDifficultyModEnabled() &&
        (difficulty >= GAMETYPES::DIFFICULTY_NUM))
    {
        difficulty = GAMETYPES::DIFFICULTY_NORMAL;
    ...
    if (IsDifficultyModEnabled() &&
        (difficulty == GAMETYPES::DIFFICULTY_VERY_HARD))
    {
        m_OnOff[Line].TextId = GAMETEXT_EMPTY;
        m_OnOff[Line].Text = VERY_HARD_TEXT;
    }
    else if (IsDifficultyModEnabled() &&
    ...
    if (m_bSwitchMode)
    {
        const int32 difficultyOptionCount =
            (IsDifficultyModEnabled() ?
             GAMETYPES::DIFFICULTY_OPTION_NUM : GAMETYPES::DIFFICULTY_NUM);
        ...
        if (CController::GetDigitalTrigger(virtualPad, CController::DIGITAL_LLEFT))
        {
            ...
            m_fSwitchMoveL = SWITCH_ANM;
            difficulty = InvClamp(difficulty - 1, 0, difficultyOptionCount - 1);
        }
        else if (CController::GetDigitalTrigger(virtualPad, CController::DIGITAL_LRIGHT))
        {
            CGameSound::PlaySE(SDCODE_SE(0x1004));

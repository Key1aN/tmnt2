# TMNT2 Slashuur Adaptive Teleport v2 Radius25 No-Flying US
...
- a ground-enemy archetype rather than one of the game's flying-enemy classes;
- within **25.0 world units** of Slashuur.

The flying-enemy exclusion covers every character derived from the game's
`CBaseFlyingEnemyChr` family. Because this build disables C++ RTTI, the filter
uses their stable `ENEMYID` values:

- Triceraton Flying Harness;
- Blood Sucker;
- Poison Bat;
- Hypnosis Bat;
- Bomb Bat Mech;
...
These enemies are ignored even when they are the nearest or only visible
enemy. A normal ground enemy briefly launched into the air remains eligible;
the exclusion is based on enemy archetype, not temporary damage animation.
...
- **Project:** `TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US`
- **Target:** US/NA Win32 Release
- **Archive changes:** none
- **Required game data:** keep the existing final modded `TMNT.DAT`
- **Base:** cumulative Adaptive Teleport v1 US / Intro Skip v1 US baseline
...
```text
MOVE teleport target selected enemy=... handle=... distance=... radius=25.000 flying_skipped=...
...
```text
MOVE teleport target none radius=25.000 flying_skipped=... fallback=forward
...
```text
MOVE teleport target lost enemy=... expected_handle=... reason=invalid_flying_or_replaced fallback=forward
```
...
2. Select **Build TMNT2 Slashuur Adaptive Teleport v2 Radius25 No-Flying US**.
3. Open the newest successful run from
   `feature/slashuur-adaptive-teleport-v2-radius25-no-flying-us`.
4. Download the artifact beginning with
   `TMNT2-Slashuur-Adaptive-Teleport-v2-Radius25-NoFlying-US-run-`.
5. Confirm that it contains:
   - `TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US.exe`
   - `TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US.pdb`
   - `TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US.map`
   - `BUILD_INFO_TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US.txt`
...
3. Open **Current branch**, search for
   `feature/slashuur-adaptive-teleport-v2-radius25-no-flying-us`, and select it.
...
3. Copy `TMNT2_Slashuur_Adaptive_Teleport_v2_Radius25_NoFlying_US.exe` into the game directory.
...
1. With no enemy nearby, confirm Slashuur still teleports about five units
   forward.
2. Put one visible enemy within 25 units, face any direction, and teleport.
   Confirm Slashuur appears behind the enemy and adopts its facing.
3. Put a bat or another listed flying enemy within 25 units and confirm
   Slashuur uses the forward fallback instead of targeting it.
4. Put a flying enemy closer than a valid ground enemy and confirm Slashuur
   ignores the flyer and appears behind the ground enemy.
5. Move the visible ground enemy beyond 25 units and confirm the forward
   fallback.
6. Put a nearby ground enemy off screen and confirm the forward fallback.
7. Put two visible ground enemies within 25 units and confirm the nearer one is
   used.
8. Confirm normal combo and charged-attack cancel still work.
9. Confirm damage, knockdown, grab, and stun states still block the cancel.
10. Exit normally to flush the trace and attach
